...
namespace velodyne_rawdata
{
      ...
      for (int j = 0, k = 0; j < SCANS_PER_BLOCK; j++, k += RAW_SCAN_SIZE) {
        ...
        if ((raw->blocks[i].rotation >= config_.min_angle 
          ...
          if (pointInRange(distance)) {
            data.addPoint(x_coord, y_coord, z_coord, corrections.laser_ring, raw->blocks[i].rotation, distance, intensity, 0);
          }
        }
      }
    }
  ...
  void RawData::unpack_vlp16(const velodyne_msgs::VelodynePacket &pkt, DataContainerBase& data)
  {
    ...
    const raw_packet_t *raw = (const raw_packet_t *) &pkt.data[0];
    uint8_t mode = raw->status[0];
    int dblock=mode==VLP16_MODE_DUAL?2:1;
    ...
    for (int block = 0; block < BLOCKS_PER_PACKET; block++) {
      ...
      if (block < (BLOCKS_PER_PACKET-dblock)){
	raw_azimuth_diff = raw->blocks[block+dblock].rotation - raw->blocks[block].rotation;
	...
	if(raw_azimuth_diff < 0)//raw->blocks[block+1].rotation - raw->blocks[block].rotation < 0)
      ...
      for (int firing=0, k=0; firing < VLP16_FIRINGS_PER_BLOCK; firing++){
        for (int dsr=0; dsr < VLP16_SCANS_PER_FIRING; dsr++, k+=RAW_SCAN_SIZE){
          ...
          /** GPS Time Stamp **/
          time = raw->time+(uint32_t)((block/dblock)*VLP16_BLOCK_TDURATION+firing*VLP16_FIRING_TOFFSET+dsr*VLP16_DSR_TOFFSET);
          
          for (int echocnt=0; echocnt<dblock; echocnt++) {
            /** Position Calculation */
            union two_bytes tmp;
            tmp.bytes[0] = raw->blocks[block+echocnt].data[k];
            tmp.bytes[1] = raw->blocks[block+echocnt].data[k+1];
            ...
            if ((azimuth_corrected >= config_.min_angle 
              ...
              intensity = raw->blocks[block+echocnt].data[k+2];
